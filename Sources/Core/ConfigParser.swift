...
public enum ConfigParser {
    ...
    public static func parse(
        ...
        while i < args.count {
            ...
            case "--port":
                ...
                guard let port = Int(v), (1...65535).contains(port) else {
                    throw BgBgOneError.parser("--port must be 1..65535, got: \(v)")
                }
            ...
            case "--token":
                ...
                guard !token.isEmpty else {
                    throw BgBgOneError.parser("--token cannot be empty")
                }
            ...
            case "--max-body-mb":
                ...
                guard let mb = Int(v), (1...512).contains(mb) else {
                    throw BgBgOneError.parser("--max-body-mb must be 1..512, got: \(v)")
                }
        ...
        if let m = explicitMode {
            ...
            if m == .serverRequested {
                if sawProcessingOption {
                    throw BgBgOneError.userError("--server cannot be combined with image-processing options")
                }
                if !cfg.inputs.isEmpty {
                    throw BgBgOneError.userError("--server does not accept image input arguments; send images to the HTTP API")
                }
                return cfg
            }
            return cfg
        ...
        if sawServerOnlyFlag && !sawServer {
            throw BgBgOneError.userError("server options require --server")
        }
