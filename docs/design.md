...
1. **Unit** (`Tests/bgbgoneTests/`, pure Swift runner) — arg parsing, colour parsing, output naming, format inference, JSON escaping, routing validation, threshold/padding config, NetworkPolicy.
2. **Integration** (`Tests/integration/run.sh`) — spawns the built binary; pipe in / pipe out / file in / file out; exit codes; JSON shape; capability gating; algorithm outputs against fixtures.
...
Current local measurement: 100 images in 8.098s, 12.35 images/s, 81.0 ms/image.
