...
```bash
bgbgone red-panda.jpg --bg "image:red-panda.jpg" --filter "fg:scale=0.75" --size preview -o red-panda-scale.jpg
```
...
![red-panda after `fg:scale=0.75` — CLI render = server render (byte-identical)](../images/filters/scale.jpg)
...
```bash
bgbgone yoga.jpg --type person --bg color:#1a2233 --filter "fg:scale=0.7"
```
...
```bash
bgbgone woman-singer.jpg --type person --bg color:#1a2233 --filter "fg:scale=0.7"
```
